...
public class MatrixManipulation	
{
    ...
    public static bool[,] MakeBoolGrid(Cell[,] mirroredCells)
    {
        var boolGrid = new bool[mirroredCells.GetLength(0), mirroredCells.GetLength(1)];
        for (var x = 0; x < mirroredCells.GetLength(0); x++)
        {
            for (var y = 0; y < mirroredCells.GetLength(1); y++)
            {
                if (mirroredCells[x, y] == null) continue;
                boolGrid[x, y] = mirroredCells[x, y].IsAlive;
            }
        }
